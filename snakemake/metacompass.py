...
rule pilon_contigs:
    ...
    shell:"java -Xmx64G -jar %s/bin/pilon-1.23.jar --flank 5 --threads {threads} --mindepth {params.mincov} --genome {input.contigs} --unpaired {input.sam} --output %s/%s.%s.assembly.out/contigs.pilon --fix bases,local 1>> {log} 2>&1;cp %s/%s.%s.assembly.out/contigs.pilon.fasta %s/%s.%s.assembly.out/contigs.final.fasta"%(config["mcdir"],config['prefix'],config['sample'],config['iter'],config['prefix'],config['sample'],config['iter'],config['prefix'],config['sample'],config['iter'])
