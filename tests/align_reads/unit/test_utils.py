...
def test_extract_reads_mapped(test_reads_unit, tmp_dir):
    ...
    assert num_lines == 259176
    assert cmp(expected_mapped_fq, out_reads_fq, shallow=False)

    # running it again to check if num_lines only reflects the new lines written and not the total in the file
    num_lines = extract_reads(
        input_sam.resolve().as_posix(),
        read_ids.resolve().as_posix(),
        interleaved_reads.resolve().as_posix(),
    ...
    assert num_lines == 259176
    assert not cmp(expected_mapped_fq, out_reads_fq, shallow=False)
...
def test_move_reads(tmp_dir):
    src = tmp_dir / "source.fq"
    dst = tmp_dir / "dst.fq"
    src.touch()
