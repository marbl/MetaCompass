...
def main():
	...
	for gene in cognames:
		...
		filehandle_cogs_fna[gene] = open(wrkdir+'marker_index'+version+'/'+cognames[gene]+'_'+gene+'/'+cognames[gene]+'_'+gene+'.fna', 'w')
		filehandle_cogs_faa[gene] = open(wrkdir+'marker_index'+version+'/'+cognames[gene]+'_'+gene+'/'+cognames[gene]+'_'+gene+'.faa', 'w')
		filehandle_cogs_genome[gene] = open(wrkdir+'marker_index'+version+'/'+cognames[gene]+'_'+gene+'/'+cognames[gene]+'_'+gene+'_genome.tsv', 'w')
		...
		with open(refseq_file) as f:
			for num, line in enumerate(f):
				...
				try:
					...
					os.system("unzip "+ wrkdir + "marker_index"+version+"/ncbi_dataset.zip -d "+ wrkdir+ "marker_index"+version)
				...
				for ff in fiternuc:
					...
					fgr.write('\t'.join([seq_id, accession, protein_name, str(nuclen), str(aalen), str(matchflag), taxid, header, str(bac_arc_ind)]) + '\n')
					fgn.write('>' + seq_id +'\n' + nucseq +'\n')
					fga.write('>' + seq_id + '\n' + corr_protein + '\n')
				fgr.close()
				fgn.close()
				...
				try:
					fetchmg_command = sys.argv[1] + "/fetchMG/fetchMG.pl -m extraction -v " + gene_aa_filename + " -o " + output_folder
				...
				with open(report_filename) as f:
					for line in f:
						...
						if val[0] in genesofinterest:
							...
							filehandle_cogs_genome[gene].write(val[0]+'\t'+cognames[gene]+'_'+gene+'\t'+val[1]+'\t'+val[3]+'\n')
